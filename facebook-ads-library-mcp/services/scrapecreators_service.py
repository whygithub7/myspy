...
def search_ads_by_keyword(
    ...
    logger.error(f"DEBUG_SCRAPE_CREATORS_SEARCH query={query!r} params={params}")
        ...
        try:
            ...
            if isinstance(resJson, dict):
                logger.error(
                    ...
                    f"searchResults_len={len(resJson.get('searchResults', []) or [])} "
                    f"keys={list(resJson.keys())}"
                )
...
def parse_fb_ads(resJson: Dict[str, Any], trim: bool = True, filter_inactive: bool = True) -> List[Dict[str, Any]]:
    ...
    for ad in results:
        try:
            ...
            media_type = snapshot.get('display_format') or ""
            ...
            # Parse media URLs based on type (be liberal in allowed formats)
            media_urls = []
            if media_type in ('IMAGE', 'MULTI_IMAGES'):
                ...
                for vid in videos:
                    url = vid.get('video_sd_url') or vid.get('video_hd_url')
                    if url:
                        media_urls.append(url)
            ...
            # Fallback: if media_urls still empty but there are images/videos fields, try to use them
            if not media_urls:
                images = snapshot.get('images') or []
                videos = snapshot.get('videos') or []
                for img in images:
